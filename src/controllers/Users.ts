...
export const getUserOrders = async (req: IUserRequest, res: Response, next: NextFunction) => {
    ...
    let orders = await Order.find(query).sort({ date: 'descending' }).skip(skip).limit(10);
    const ordrsCount = await Order.find(query).count();
    ...
    return res.status(200).json({ status: 200, pagesNumber, summary: { ...ordersSummary[0], usersCount, restFlour, totalAccount, restAccount }, orders });
}
